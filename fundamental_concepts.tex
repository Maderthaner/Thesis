...
\intertext{where $e$ is the elementary charge, $B_{\text{max}}$ is the maximum magnetic field in the undulator or wiggler, $m_{e}$ is the mass of an electron, $c$ is the speed of light, and $\lambda_{U}$ is the so-called undulator period that corresponds to the magnet spacing. $K$ can be expressed in convenient units}
