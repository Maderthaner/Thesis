...
\begin{itemize}
	\item In the range $\num{e4} < \Gamma^{*} < \num{e6}$, $\Gamma^{*}$ becomes
		...
		\end{equation}
\end{itemize}
Note, however, that for larger cluster sizes, the Equation \eqref{eq:large-hagena-scaling} becomes true again \citep{Hagena-1992-RSI,Bush-1998-JPhysChemA}. The average number of atoms, $\left\langle N\right\rangle$, which arrange to a cluster, is in this thesis usually expressed as the radius of a cluster. To transform the number of atoms, $\left\langle N\right\rangle$, to a radius, a good approximation can be achieved by using the atomic Wigner-Seitz radius, $R_{s}$. It can be noted as \cite{Arbeiter-2010-PRA,Gomez-2011-JCP}
\begin{align}
R_{s} &= \left(\frac{3 M}{4 \pi\, \rho\, N_{A}}\right)^{\frac{1}{3}},
\label{eq:wigner-seitz-radius}
\intertext{where $M$ is the molar mass, $\rho$ is the mass density, and $N_{A}$ is the Avogadro number. The cluster radius, $r$, can then be noted as,}
r &= R_{s} \left\langle N\right\rangle^{\frac{1}{3}},
\end{align}
which is typically given in \AA. Using the mass density for superfluid He-droplets, $R_{s}^{He}\approx \SI{2.2}{\angstrom}$ and for solid xenon, $R_{s}^{Xe} \approx \SI{1.7}{\angstrom}$. Note, that this varies with temperature and phase state quite drastically.\\[1\baselineskip]
%
...
Nanoparticles respond to intense X-ray radiation by scattering or absorbing the rays. Figure \ref{fig:absorption-cross-section-carbon-ken} shows the most probable processes in the X-ray wavelength range. For soft X-rays, the processes can be reduced to the photoionization process and the coherent and elastic scattering. Therefore, Section \ref{sec:absorption} discusses photoionization and the resulting core-hole decay processes; Section \ref{sec:saxs} discusses the coherent and elastic scattering of an electron and extends this concept to the scattering and imaging of a nanoparticle; and Section \ref{sec:generalized-index-of-refraction} generalizes the refractive index. The chapter follows the discussion in \citep{Als-Nielson-2011-JWS} and avoids effects that have negligible cross-sections, such as the Compton effect, the pair-production, and the scattering from the nucleus.
...
\begin{table}
	\centering
		...
		\hline
			...
			and ionized subshell & $\lvert-r_{0}f^{0}\rvert^2$ in barn \\ \hline
		...
		\end{tabular}
	\caption[Atomic scattering factors for helium and xenon.]{Atomic scattering cross-sections $\lvert-r_{0}f^{0}\rvert^2$ for certain electron configurations. From \citep{Ho-2016-PC}.}
